  ...
  Mutation: {
    ...
    deleteCuenta:async (_, {id}) => {
      try {
        ...
        if(row!==undefined){
        const {affectedRows} = await pool.query("DELETE FROM `CONTABILIDAD` WHERE `CONTABILIDAD`.`ID` = ?", id);
        if(affectedRows>0)
        return true;
        else
        return false;}
    ...
    editCuenta:async (_, {input,id,file}) => {
      ...
      updateDataReady=updateDataReady?updateDataReady:Object.assign(row,input)

      const {affectedRows} = await pool.query("UPDATE CONTABILIDAD SET ? WHERE ID=?",[updateDataReady,id]);      
